...
def main():
    ...
    clients = Clients(lambda : ConvNet(input_size = data_info["input_size"], channels = data_info["num_channels"], \
            num_classes=data_info["num_classes"]), lambda x: torch.optim.Adam(x, lr=args.lr), \
                args, client_train_datasets, client_valid_datasets, client_test_datasets)
