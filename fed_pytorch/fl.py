...
def run_fl(clients, args):
    ...
    for epoch in range(1, args.global_epochs + 1):
        ...
        for c_id in tqdm(random_clients, ascii=True):
            ...
            # Train one client
            clients.train_epoch(c_id, epoch, args, global_weights)
            ...
            if client_weights_sum is None:
                client_weights_sum = dict(
                    (key, value * clients.client_train_prop[c_id])
                    for key, value in current_client_weights.items())
            else:
                for key in client_weights_sum.keys():
                    client_weights_sum[key] += \
                        clients.client_train_prop[c_id] \
                        * current_client_weights[key]
        ...
        if args.valid_frac > 0 and epoch % args.eval_interval == 0:
            ...
            for c_id in tqdm(range(args.n_clients), ascii=True):
                if c_id in random_clients:
                    eval_log = clients.evaluation(c_id, mode="valid")
                else:
                    eval_log = clients.get_old_eval_log()
                eval_logs.append(dict((key, value
                                       * clients.client_valid_prop[c_id])
                                      for key, value in eval_log.items()))

            evaluation_logging(eval_logs, epoch, mode="valid")

    clients.set_global_weights(global_weights)
    eval_logs = []
    for c_id in tqdm(range(args.n_clients), ascii=True):
        eval_log = clients.evaluation(c_id, mode="test")
        eval_logs.append(dict((key, value * clients.client_test_prop[c_id])
                              for key, value in eval_log.items()))

    evaluation_logging(eval_logs, epoch, mode="test")
