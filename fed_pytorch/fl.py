...
def run_fl(clients, args):
    ...
    if args.fed_method == "Ditto":
        ...
        client_per_params = [clients.get_params()
    ...
    for round in range(1, args.rounds + 1):
        ...
        for c_id in tqdm(random_clients, ascii=True):
            ...
            if client_params_sum is None:
                client_params_sum = dict(
                    (key, value * clients.client_train_weight[c_id])
                    for key, value in current_client_params.items())
            else:
                for key in client_params_sum.keys():
                    client_params_sum[key] += \
                        clients.client_train_weight[c_id] \
            ...
            if args.fed_method == "Ditto":
                ...
                clients.train_epochs(
                    c_id, round, args, global_params, per=True)
